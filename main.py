...
def create_dirs():
    ...
    if not os.path.exists('./results/examples/Egogesture/train/'):
        os.makedirs('./results/examples/Egogesture/train/')

    if not os.path.exists('./results/examples/Egogesture/test/'):
        os.makedirs('./results/examples/Egogesture/test/')
    ...
    if not os.path.exists('./npy/Egogesture/'):
        os.makedirs('./npy/Egogesture/')
...
def init_params(target):
    if target == 'MNIST':
        ...
        train_dataset = torchvision.datasets.ImageFolder(root='E:\Research\GAN_Training_EgoGesture\DualClass_Training2\Train',
                                    transform=transforms.Compose([
        ...
        test_dataset = torchvision.datasets.ImageFolder(root='E:\Research\GAN_Training_EgoGesture\DualClass_Training2\Test',
                                    transform=transforms.Compose([
                                        transforms.Resize(size=(112, 112)),
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
...
def test_attack_performance(target, dataloader, mode, adv_GAN, target_model, batch_size, l_inf_bound, dataset_size):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        for j in range(adv_img.shape[0]):
            ...
            if target == 'HighResolution' or target == 'Egogesture':
